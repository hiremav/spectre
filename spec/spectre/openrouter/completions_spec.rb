...
RSpec.describe Spectre::Openrouter::Completions do
  ...
  before do
    Spectre.setup do |config|
      config.default_llm_provider = :openrouter
      config.openrouter do |or_cfg|
        or_cfg.api_key = api_key
        # optional headers are not required for tests
      end
    end
  end
  ...
  describe '.create' do
    ...
    context 'when the API returns an error' do
      ...
      it 'raises an error with the API response' do
        expect {
          described_class.create(messages: messages)
        }.to raise_error(RuntimeError, /OpenRouter API Error/)
      end
    end
    ...
    context 'when the response finish_reason is model_length' do
      let(:model_length_response_body) { { choices: [{ message: { content: completion }, finish_reason: 'model_length' }] }.to_json }

      before do
        stub_request(:post, Spectre::Openrouter::Completions::API_URL)
          .to_return(status: 200, body: model_length_response_body, headers: { 'Content-Type' => 'application/json' })
      end
    ...
    context 'when the response contains a tool call via function_call or tool_calls' do
    ...
    context 'when finish_reason is error' do
      let(:error_finish_response_body) { { choices: [{ message: { content: '' }, finish_reason: 'error' }] }.to_json }

      before do
        stub_request(:post, Spectre::Openrouter::Completions::API_URL)
          .to_return(status: 200, body: error_finish_response_body, headers: { 'Content-Type' => 'application/json' })
      end

      it 'raises an error describing the model finish_reason=error' do
        expect {
          described_class.create(messages: messages)
        }.to raise_error(RuntimeError, /Model returned finish_reason=error/)
      end
    end
    ...
    context 'with a json_schema parameter' do
      let(:json_schema) { { name: 'completion_response', schema: { type: 'object', properties: { response: { type: 'string' } }, required: ['response'] } } }
      ...
      it 'sends the json_schema in the request under response_format.json_schema' do
